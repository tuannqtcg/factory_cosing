...
- ~~Custom claim `role` trên Firebase Auth user~~ — XONG (**ADR-017**,
  2026-07-10): Cloud Function `setUserRole` (onCall, admin-only) +
  `roleAudit/{entryId}` (append-only, chỉ admin đọc) + `scripts/bootstrap-admin.ts`
  cho admin đầu tiên. `scripts/seed-emulator.ts` (gọi Admin SDK trực tiếp) vẫn
  CHỈ dùng cho Emulator — không đổi.
